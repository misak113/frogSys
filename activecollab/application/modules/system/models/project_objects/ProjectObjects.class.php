<?php
  ...
  class ProjectObjects extends BaseProjectObjects {
    ...
    function groupByMilestone($objects, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL) {
       ...
       * @return array
       */
      function groupByCategory($project, $milestones, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL) {
          $result = array();

          if (!is_foreachable($milestones))
              return $result;

          foreach ($milestones as $milestone_id => $milestone) {
              if(!is_foreachable($milestone['objects']))
                continue;

              $resultCategory = array();

              $objects = $milestone['objects'];

              $category_ids = objects_array_extract($objects, 'getParentId');
              if(is_foreachable($category_ids)) {
                  $categories = Categories::findByProject($project);
                  if(is_foreachable($categories)) {
                      foreach($categories as $category) {
                          $resultCategory[$category->getId()] = array(
                              'category' => $category,
                              'objects' => array(),
                          );
                      } // foreach
              ...
              // For unknown milestone objects
              $resultCategory[0] = array(
                  'category' => null,
                  'objects' => array(),
              );

              foreach($objects as $object) {
                  if(isset($resultCategory[$object->getParentId()])) {
                      $resultCategory[$object->getParentId()]['objects'][] = $object;
                  } else {
                      $resultCategory[0]['objects'][] = $object;
                  } // if
              } // foreach

              $result[$milestone_id] = array(
