...
function addImages(page_part) {
	...
	changeWindow('jumpLoaderApplet', false, 600, false, false);
	document.getElementById('jumpLoaderApplet').style.width = "100%";
}
...
function checkboxSelectGalerie(id, page_part) {
	...
	if (obj.style.opacity == 0.1) {
		...
		changeOpacity("image_"+id, 0.1);
		//obj.style.opacity = "0.3";
		var show = 0;
	}
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=galerie&action=show&show="+show+"&id="+id, createAlert)) {
...
function deleteGalerie2(id) {
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=galerie&action=delete&id="+id, partRefresh)) {
		//pokud nefunguje ajax
		return;
	}
}
...
function saveGalerie(id) {
	...
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=galerie&action=edit&id="+id+"&title="+fixQuery(title), createAlert)) {
		//pokud nefunguje ajax
		return;
	}
	zavriWindow();
...
function saveGaleriePosition(id, changeWith) {
	...
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=galerie&action=sort&id="+id+"&change_with="+changeWith, partRefresh)) {
		//pokud nefunguje ajax
		return;
	}
}
