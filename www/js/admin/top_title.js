...
function saveTop_title() {
	...
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=top_title&action=save&text="+fixQuery(text), refreshTop_title)) {
...
function refreshTop_title(text) {
	if (!postAjaxRequest("/bin/ajax/top_title.php", "", refreshTop_title2)) {
		//pokud nefunguje ajax
		return;
	}
