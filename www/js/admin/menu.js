...
function saveMenuPosition(id, changeWith) {
	...
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=menu&action=sort&id="+id+"&change_with="+changeWith, menuEdited)) {
...
function deleteMenu2(id) {
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=menu&action=delete&id="+id, menuEdited)) {
		//pokud nefunguje ajax
		return;
	}
}
...
function editMenu(id) {
	idEditedMenu = id;
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=menu&action=edit_form&id="+id, editMenu2)) {
		//pokud nefunguje ajax
		return;
	}
}
...
function saveMenu(id) {
	...
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=menu&action=edit&id="+id+"&name="+fixQuery(nazev)+"&link="+fixQuery(odkaz)+"&visible="+visible, menuEdited)) {
		//pokud nefunguje ajax
		return;
	}
	zavriWindow();
...
function addMenu(id) {
	if (!postAjaxRequest("/bin/ajax/edit.php", "predmet=menu&action=add&id="+id, menuAdded)) {
		//pokud nefunguje ajax
		return;
	}
}
