<?php
			if ($_POST['action'] == "add") {
				...
				if ($res = mysql_fetch_array($q)) {
					...
					$sql = "SELECT * FROM `page` WHERE `first` <> '".$_POST['parent']."' AND `parent` = ".$res3['parent']."";
					$q4 = mysql_query($sql);
			...
			if ($_POST['action'] == "edit") {
				...
				if ($q = mysql_query($sql)) {
					echo "Položka menu byla úspěšně uložena.";
				} else {
					...
					<tr>
						<td>
							...
							<a href="javascript: zobrazHref(<?php echo $res['id']; ?>);;">
								<img src="/images/icons/oko.png" alt="zobraz" class="zobraz" />
							</a>
							<a href="javascript: nastavHref(<?php echo $res['id']; ?>);">
								<img src="/images/icons/set.png" alt="nastav" class="nastav" />
							</a>
							</div>
						</td>
						<td>
							<span style="color: <?php echo $color; ?>; font-size: <?php echo $size; ?>px;">
								<?php                          
								$pole = file(URL."bin/ajax/subPage.php?page_part=".$res['id']);
