<?php
			if ($_POST['action'] == "add") {
				...
				$sql = "INSERT INTO `plan_akci` VALUES(NULL, '$rand', NOW(), NOW(), '', '', '', NOW(), ".$_POST['parent'].", 0, 0)";
				$q = mysql_query($sql);
			...
			?>
				<table class="edit_table">
					...
					<tr>
						...
						<td>
							<textarea id="plan_akci_popis_<?php echo $res['id']; ?>">
								<?php echo $res['text']; ?>
							</textarea>
						</td>
					</tr>
			...
			if ($_POST['action'] == "save") {
				$sql = "UPDATE `plan_akci` SET `name` = '".$_POST['name']."', `kdy` = '".$_POST['kdy']."', `kde` = '".$_POST['kde']."', `co` = '".$_POST['co']."', `text` = '".$_POST['text']."', `do` = '".$_POST['do']."', `limit_lidi` = ".$_POST['limit_lidi'].", `cena` = ".$_POST['cena'].", `time` = NOW() WHERE `id` = ".$_POST['id']."";
			...
			if ($_POST['action'] == "mail_user") {
				...
				if ($res = mysql_fetch_array($q)) {
					...
					//$headers .= "To: ".$res['email'].", ".ADMIN_MAIL."\n"; 
					$headers .= "Subject: =?iso-8859-2?B?".base64_encode("Záloha na akci - ".$akce)."?=\n";
					//$headers .= "Date: ".Time()."\r\n";
					...
					imap_mail("".$res['email'].", ".ADMIN_MAIL."",
						"",
						"Dobrý den ".$res['jmeno']." ".$res['prijmeni'].",\n".
						"Byla obdržena záloha za akci.\n".
			...
			if ($_POST['action'] == "send_collective_mail") {   
				...
				while ($res = mysql_fetch_array($q)) {
					...
					$headers .= "Content-Type: text/plain; charset=utf-8\r\n";
					$headers .= "From: =?iso-8859-2?B?".base64_encode("Info")."?= <".ADMIN_MAIL.">\n"; 
					//$headers .= "To: ".$mail."".ADMIN_MAIL."\n"; 
					$headers .= "Subject: =?iso-8859-2?B?".base64_encode($_POST['predmet_mail'])."?=\n";
					//$headers .= "Date: ".Time()."\r\n";
					$headers .= "Reply-To: ".ADMIN_MAIL." <".ADMIN_MAIL.">\n";
					$headers .= "Return-Path: ".ADMIN_MAIL." <".ADMIN_MAIL.">\r\n"; 
					$headers .= "X-Priority: 2\n"; 
