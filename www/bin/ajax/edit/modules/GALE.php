<?php
	...
	if (@$_SESSION['auth'] > 0) {
		...
		$sql = "INSERT INTO `galerie` VALUES(NULL, '#new.".$file_name."', '', ".$_GET['parent'].", 1, $order)";
		$q = mysql_query($sql);
		
		$sql = "SELECT * FROM `galerie` WHERE `name` = '#new.".$file_name."'";
		$q = mysql_query($sql);
		if ($res = mysql_fetch_array($q)) {
			copy(PATH."/userfiles/galerie/temp/".$file_name."small.jpg", PATH."/userfiles/galerie/thumbs/".$res['id'].".".$file_name.".jpg");
        	copy(PATH."/userfiles/galerie/temp/".$file_name."large.jpg", PATH."/userfiles/galerie/".$res['id'].".".$file_name.".jpg");
			unlink(PATH."/userfiles/galerie/temp/".$file_name."small.jpg");
   	    	unlink(PATH."/userfiles/galerie/temp/".$file_name."large.jpg");
			$sql = "UPDATE `galerie` SET `name` = '".$file_name.".jpg' WHERE `id` = ".$res['id']."";
			$q = mysql_query($sql);
		}
	}
	}
