	...
	function writeGalerie($page_part) {
		...
		echo '
			<a href="javascript: addImages('.$page_part.');">
				<img src="/images/icons/addImages.png" alt="Vložit obrázky" />
			</a>
			<applet id="jumpLoaderApplet" name="jumpLoaderApplet"
					code="jmaster.jumploader.app.JumpLoaderApplet.class"
					archive="/java/jump_loader/mediautil_z.jar,/java/jump_loader/sanselan_z.jar,/java/jump_loader/jumploader_z.jar"
					width="0"
					height="0"
					mayscript>
					...
					<param name="uc_uploadUrl" value="/bin/ajax/edit/modules/GALE.php?predmet=galerie&action=add_images&parent='.$page_part.'"/>
		...
		while ($res = mysql_fetch_array($q)) {
			if ($res['show'] == 1 || @$_SESSION['auth'] > 0) {
				...
				if (@$_SESSION['auth'] > 0) {
					if ($res['show'] == 0) {
						$check = "C";
						$trans = "style=\"opacity: 0.1; -moz-opacity: 0.1;\"";
					} else {
						$check = "Č";
						$trans = "";
					}
				...
				echo '	<a href="/userfiles/galerie/'.$res['id'].'.'.$res['name'].'" rel="lightbox[roadtrip]">
						<img '.$trans.' src="/userfiles/galerie/thumbs/'.$res['id'].'.'.$res['name'].'" alt="'.$res['title'].'" title="'.$res['title'].'" id="image_'.$res['id'].'" class="galerie" />
					</a>
					';
				if (@$_SESSION['auth'] > 0) {
					echo '
