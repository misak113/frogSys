<?php
	...
	function writePlanAkci($page_part) {
		if ($_POST['prihlasit_akce'] != "" && $_POST['jmeno'] != "" && $_POST['prijmeni'] != "" && $_POST['email'] != "") {
			...
			if ($prihlaseno < $max) {
				...
				$q = mysql_query($sql);
					...
					$headers .= "From: =?iso-8859-2?B?".base64_encode("Info")."?= <".ADMIN_MAIL.">\n"; 
					//$headers .= "To: ".$_POST['email'].", ".ADMIN_MAIL."\n"; 
					$headers .= "Subject: =?iso-8859-2?B?".base64_encode("Přihlášení na akci - ".$akce)."?=\n";
					//$headers .= "Date: ".Time()."\r\n";
					$headers .= "Reply-To: ".ADMIN_MAIL." <".ADMIN_MAIL.">\n";
					$headers .= "Return-Path: ".ADMIN_MAIL." <".ADMIN_MAIL.">\r\n"; 
				...
				imap_mail("".$_POST['email'].", ".ADMIN_MAIL."",
					"",
					"Dobrý den ".$_POST['jmeno']." ".$_POST['prijmeni'].",\n".
					"Byl jste přihlášen na uvedenou akci.\n".
					"Prosím zaplaťte zálohu ve výši 500kč na účet 000 000 000 / 0800\n".
					"Jako variabilní symbol uveďte: ".$vs."",
					$headers
					);
		...
		include PATH."/bin/plugins/kalendar.php";
	...
	<div class="polozka_plan_akci">
		<div class="hlavicka_plan_akci">
			...
			<div class="podrobnosti_plan_akci">
				<a href="javascript: openPodrobnost(<?php echo $res['id']; ?>);">
					<img src="/images/icons/podrobnosti_open.png" alt="podrobnosti" id="podrobnosti_img_plan_akci_<?php echo $res['id']; ?>" width="15" height="15">
		...
		<div class="telo_plan_akci" id="telo_plan_akci_<?php echo $res['id']; ?>" style="height: 1px;">
			<div class="atribut_plan_akci">
				...
				<a href="javascript: showKalendar('kalendar_<?php echo $res['id']."_user"; ?>')">
					<img src="/images/icons/calendar_day.png" alt="kalendář" class="kalendar">
				</a>
			</div>
			<div class="atribut_plan_akci">
					...
					<table>
						...
						<tr>
							<td colspan="2">
								...
								<p id="prihlaseni-info">Pro závazné přihlášení na akci je nutné poslat poslat 500 kč na učet 00000000/0000. s variabilním symbolem 00000. Po obdržení Vám bude zaslán potvrzující email. Více info na <a href="mailto:info@janarandakova.cz">info@janarandakova.cz</a>
                <br /><a title="Podmínky a postup platby" href="/plan-akci/obchodni-podminky/">» Podmínky a postup platby</a>
                </p>
...
<?php
		...
		if (@$_SESSION['auth'] > 0) {
		?>
			<a href="javascript: addAkce(<?php echo $page_part; ?>);"><img src="/images/icons/add.png" alt="add" class="add_menu_in"></a>
		<?php
		}
