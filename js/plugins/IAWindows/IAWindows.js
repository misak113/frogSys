...
loadScript("/IAWindows/prechody.js");
loadScript("/IAWindows/windows.js");
...
var aktual_title;
function showInfo(event, text, el) {
	...
	changeOpacity("info_box", 1);
        aktual_title = el.getAttribute("title");
        el.setAttribute("title", "");
	el.setAttribute("onmouseout", "hideInfo(this);");
        el.setAttribute("onmousemove", "reshowInfo(event);");
}
...
function hideInfo(el) {
	...
	hidingTime = setTimeout("document.getElementById('info_box').style.display = 'none'", 20*20);
        el.setAttribute("title", aktual_title);
}
