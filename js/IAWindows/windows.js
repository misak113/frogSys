...
function createEmptyWindow() {
	var win = ''+
		...
		'					<a href="javascript: zavriWindow();"><img src="'+URL+'frogSys/images/window/close.png" class="window_close" alt="zavřít" /></a>'+
		'				</td>'+
		...
		'	</div>'+
		'</div>'+
		'<div id="info_box" onmouseover="hideInfo(this);">'+
		'</div>'+
		'';
...
function createAlert(text, x, y) {
	...
	document.getElementById("window_zalozni").id = "window_objekt";
        if (x == "center") {
            x = (document.body.clientWidth-document.getElementById(idWin).offsetWidth)/2+document.documentElement.scrollLeft+document.body.scrollLeft;
        } else if (x == null) {
            x = 10;
        }
        if (y == "midle") {
            y = 300/*+document.documentElement.scrollTop+document.body.scrollTop*/;
        } else if (y == null) {
            y = 40;
        }
	changeWindowQuick(idWin, 200, 60, x, y);
	changeOpacity(idWin, 1);
	var objekt = document.getElementById(idWin);
