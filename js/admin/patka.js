...
function savePatka() {
	...
	if (!postAjaxRequest(URL+"frogSys/bin/ajax/edit.php", "predmet=patka&action=save&text="+fixQuery(text), refreshPatka)) {
...
function refreshPatka(text) {
	if (!postAjaxRequest(URL+"frogSys/bin/ajax/patka.php", "", refreshPatka2)) {
		//pokud nefunguje ajax
		return;
	}
	patka_editing = false;
