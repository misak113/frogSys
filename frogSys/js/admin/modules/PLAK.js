...
var id_plan_akci_del, isAddAkce = false;
function editPlan_akci(id) {
    if (isAddAkce) {
        partRefresh("Akce přidána");
        isAddAkce = false;
    }
    id_plan_akci_del = id;
...
function copyPlan_akci(id) {
    if (!postAjaxRequest(URL+"frogSys/bin/ajax/edit.php", "predmet=plan_akci&action=copy&id="+id, partRefresh)) {
        //pokud nefunguje ajax
        return;
    }
    zavriWindow();
